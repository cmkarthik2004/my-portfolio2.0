        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-md bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 font-mono text-xs font-semibold uppercase tracking-wider mb-3">
              03 / Portfolio Showcase & Live Systems
            </div>
            <h2
            ...
            <p className="text-base sm:text-lg text-slate-600 dark:text-slate-300 mt-3">
              ...
              federated learning research, and production-deployed systems.
            </p>
          </div>

          {/* Filter Pills */}
          <div className="flex flex-wrap items-center gap-1.5 p-1.5 rounded-2xl bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/80 self-start md:self-auto">
            {filterTabs.map((tab) => (
              <button
                key={tab.id}
          ...
          {filteredProjects.map((project) => {
            ...
            return (
              <div
                ...
                className="group relative rounded-3xl bg-white dark:bg-slate-900/90 border border-slate-200/90 dark:border-slate-800 p-5 sm:p-7 flex flex-col justify-between shadow-xs hover:shadow-2xl hover:border-indigo-300 dark:hover:border-indigo-700/70 transition-all duration-300"
              >
                <div>
                  {/* Visual Project Media Area */}
                  <div
                    onClick={() => setActiveModalProject(project)}
                    className="cursor-pointer mb-5 relative overflow-hidden rounded-2xl"
                  >
                    <ProjectMediaPreview
                      project={project}
                      currentImageIndex={0}
                    ...
                    <div className="absolute inset-0 bg-slate-950/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center pointer-events-none">
                      <div className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-white/90 dark:bg-slate-900/90 text-slate-900 dark:text-white text-xs font-semibold shadow-lg backdrop-blur-sm">
                  ...
                  <div className="flex items-center justify-between gap-3 mb-2.5">
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-mono font-medium bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200/60 dark:border-slate-700/60">
                      {project.category}
                    </span>
                  ...
                  <h3
                    onClick={() => setActiveModalProject(project)}
                    className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white tracking-tight mb-2 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors cursor-pointer"
                  >
                    {project.name}
                  </h3>
                ...
                <div className="pt-2">
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-5">
                    {project.technologies.slice(0, 5).map((tech) => (
                      <span
                        key={tech}
                        className="px-2.5 py-1 rounded-md bg-slate-100 dark:bg-slate-800/80 text-slate-700 dark:text-slate-300 text-xs font-mono border border-slate-200/60 dark:border-slate-700/60"
                      >
                        {tech}
                      </span>
                    ))}
                    {project.technologies.length > 5 && (
                      <span className="px-2 py-1 text-[11px] font-mono text-slate-500 dark:text-slate-400">
                        +{project.technologies.length - 5} more
                      </span>
                    )}
                  </div>

                  {/* Action Link Row — DYNAMICALLY displays only what exists */}
                  <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-800/80">
                    ...
                    <div className="flex items-center gap-2">
                      {/* Live Website Button — Rendered ONLY if liveUrl exists */}
                      {isLive && (
                        <a
                          ...
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 text-xs font-semibold transition-colors border border-emerald-500/20"
                      ...
                      {/* GitHub Button — Rendered ONLY if githubUrl exists */}
                      {hasGithub && (
                        <a
